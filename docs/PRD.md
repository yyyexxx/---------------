...
7. 作为财务人员，提交时系统自动检查该发票号码是否已报销过，重复则报警并展示原报销记录，修改发票代码无法绕过查重。
...
### 重复检测
按「发票号码」作为唯一键查 SQLite。已存在则跳转到重复报警页，展示原报销记录（发票信息、原报销人、原报销时间），阻塞提交。

**设计决策**：仅按发票号码查重，不绑发票代码。原因是表单字段可编辑——恶意/误操作者可修改发票代码绕过发票代码+号码组合查重。发票号码本身在增值税发票体系中具有唯一性，单独使用不会产生误报。
...
4. **Duplicate Checker** — 先写入一条记录，验证同号码被检测为重复；不同号码不重复
...
| 现象 | 根因 | 解决 |
|:---|:---|:---|
| 同一张发票改发票代码后可重复提交（实测成功提交 5 次，ID=6~10） | 重复检测用表单提交值查重，表单字段可编辑，用户可改为不同代码绕过 | (1) `check_duplicate` 改为仅按 `invoice_number` 查重（不再查 `invoice_code`）；(2) 查重使用 `task_data` 原始识别值，优先于表单提交值，防止篡改 |

**15. IDM 拦截 PDF/Excel 下载**

| 现象 | 根因 | 解决 |
|:---|:---|:---|
| 点击下载 PDF/Excel 弹出 IDM 购买许可警告 | IDM 拦截 `<a href>` 触发的 `Content-Disposition: attachment` 下载 | 下载按钮改用 JavaScript `fetch` + `Blob` + `URL.createObjectURL` + 动态 `<a>` 触发下载，绕过 IDM 拦截 |
...
- [x] **安全修复**：重复检测绕过漏洞（仅按发票号码查重 + task_data 优先）
- [x] **兼容修复**：IDM 下载拦截问题（fetch+blob 绕过）
- [ ] **演示材料**：6 张核心功能截图 + 技术报告（LaTeX → PDF）
